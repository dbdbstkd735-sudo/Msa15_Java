...
public class StreamArrayList {
	...
	public static void main(String[] args) {
		...
		productList.add(new Product("피로회복제", 2000));

		// map - 매핑
		// : 스트림 요소를 사요하려는 형태로 변환하는 연산
		...
		productList.stream()
				...
				.sorted(Comparator.comparing(Product::getName)).forEach(p -> System.out.println(p));
		System.out.println();
		// 이름순 -내림차순
		...
		productList.stream()
				// .sorted( Comparator.comparing(p -> p.getName( ) ) )
				.sorted(Comparator.comparing(Product::getPrice)).forEach(p -> System.out.println(p));
		System.out.println();
		// 가격순 -내림차순
		System.out.println("이름순으로 정렬 (내림차순)");
